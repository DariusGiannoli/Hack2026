...
BASE_QUAT = np.array([-0.5, 0.5, -0.5, 0.5])  # précédent + Ry(+90°) en body frame
...
_SCENE_XML = os.path.join(_DIR, "robots", "leap_hand", "scene_inspire_hand_only.xml")
...
def _update(data:         mujoco.MjData,
        ...
        if pc is not None:
            try:
                ...
                if (err_pc == sl.ERROR_CODE.SUCCESS and
                    ...
                    # ZED X = right, Y = up → sim_x (latéral), sim_z (hauteur)
                    x_m = float(pc_val[0])
                    y_m = float(pc_val[1])
                    sim_x  = -x_m * TRANS_SCALE
                    sim_y  = START_Y + (DEPTH_MID_M - depth_m) * DEPTH_SCALE * TRANS_SCALE
    ...
    if mid_l is not None and lm_other is not None and _wrist_ref_angle is not None:
        ...
        lh2_z =  START_Z + (-(v_lh2 - cam.cy) / cam.fy * START_Y) * TRANS_SCALE
        lh2_y =  START_Y
            ...
            if pc_l is not None:
                try:
                    ...
                    if (err_lh2 == sl.ERROR_CODE.SUCCESS and
                        ...
                        lh2_y =  START_Y + (DEPTH_MID_M - depth_lh2) * DEPTH_SCALE * TRANS_SCALE
                        lh2_z =  START_Z + float(pc_lh2_val[1]) * TRANS_SCALE
                except Exception:
                    pass
                ...
                if pc is not None:
                    try:
                        ...
                        if (err_lh == sl.ERROR_CODE.SUCCESS and
                            ...
                            lh_y = START_Y + (DEPTH_MID_M - depth_lh) * DEPTH_SCALE * TRANS_SCALE
    ...
    with mujoco.viewer.launch_passive(model, data, key_callback=_key_callback) as v:
        while v.is_running():
            ...
            if g1_right_arm_ik is not None:
                # Capturer la référence au premier frame après calibration
            ...
            for _ in range(N_SUBSTEPS):
                # Geler le G1 : tronc + jambes seulement (bras = IK)
